...
export default async function LessonPage({ params }: { params: Promise<{ slug: string; lesson: string }> }) { const { slug, lesson } = await params; const found = findLesson(slug, lesson); if (!found) notFound(); return <MemberGate returnTo={`/academy/${slug}/${lesson}`} area="academy"><LessonExperience course={found.course} lesson={found.lesson} /></MemberGate>; }
